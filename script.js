...
// total amount sum 
let totalAmount = amountArray.reduce((acc, curent) => acc + curent, 0);

// max and min array value 
let max = Math.max(...amountArray);
let min = Math.min(...amountArray);

// max and min array index 
const maxIndex = amountArray.indexOf(max);
const minIndex = amountArray.indexOf(min);

//new average array with calculation the change from month to month
for (let index = 0; index < amountArray.length; index++) {
    if(amountArray[index + 1]){
        averageArray.push(amountArray[index + 1] - amountArray[index]);
    }
}

//average array sum
let average = averageArray.reduce((acc, curent) => acc + curent, 0);
